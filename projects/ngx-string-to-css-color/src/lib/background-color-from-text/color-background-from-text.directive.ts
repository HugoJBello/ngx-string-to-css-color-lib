...
export class ColorBackgroundFromTextDirective implements OnInit {
  ...
  ngOnInit() {
    if (this.colorBackgroundFromText) {
      this.el.nativeElement.style.backgroundColor = this.stringToColor.stringToColor(this.colorBackgroundFromText, this.applyShade);
    } else {
      this.el.nativeElement.style.backgroundColor = this.stringToColor.stringToColor(this.el.nativeElement.innerText, this.applyShade);
    }
